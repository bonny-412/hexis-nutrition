...
- **Paziente** (V2, esteso in V6, V8, V9, V14): nome, cognome, codice fiscale (**obbligatorio da V14**, 2026-09-02, `VARCHAR(16)`, validato lato server con `@Pattern` — 16 caratteri alfanumerici, nessun controllo del carattere di controllo/struttura per data di nascita/comune — e non univoco: due pazienti dello stesso professionista possono avere lo stesso CF per errore, nessun vincolo a DB), email, telefono, data di nascita (**obbligatoria da V8**, 2026-09-01), sesso (**obbligatorio da V9**, 2026-09-01, valori M/F/ALTRO), lavoro, tipo di lavoro (`SEDENTARIO`/`POCO_ATTIVO`/`ATTIVO`/`MOLTO_ATTIVO`); collegato a un `Professionista`; stato account (`MAI_INVITATO` / `INVITATO` / `ATTIVO`). Non richiede un account attivo per esistere. Email univoca solo tra account `ATTIVO` (indice parziale), non a livello di anagrafica. L'altezza **non** è più qui: è stata spostata su `Visita` (V5), storicizzata per ogni visita anziché unica per paziente. Nessun campo età: si calcola da `dataNascita` al volo (frontend), mai persistito, per non disallinearsi nel tempo — scelta di Andrea il 2026-09-01.
