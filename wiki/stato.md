...
Andrea ha annunciato l'ordine per la prossima sessione (3 settembre 2026, fine sessione): prima lavoro sulla **pagina di dettaglio del paziente** (`PazienteDettaglioView.vue` — scope non ancora specificato: potrebbero essere rifiniture, oppure preparazione al passo successivo; da chiarire a inizio sessione), poi la **modifica anagrafica paziente** (rotta `/pazienti/:id/modifica`, oggi un bottone disabilitato in quella stessa view). Resta aperto anche l'**inserimento nuova visita su paziente esistente** (rotta `/pazienti/:id/nuova-visita`, bottone disabilitato nella stessa view, voce di menu disabilitata nella lista pazienti fin dal 2 settembre) — non ricomparso nell'ultima indicazione di Andrea, verificare se è ancora in coda dopo la modifica anagrafica o se è stato riordinato. Dettagli su scope e comportamento non ancora raccolti, da chiedere a inizio sessione via `superpowers:brainstorming` (percorso architetturale quasi certo per "nuova visita", che riusa `DatiVisitaForm.vue` ma richiede un nuovo endpoint backend `POST /pazienti/{id}/visite`; "modifica anagrafica" potrebbe essere bounded o architetturale a seconda di quanti campi Andrea vuole editabili).

Per "nuova visita", nota di contesto utile: `DatiVisitaForm.vue` esiste già come componente autonomo (estratto apposta il 1° settembre 2026 "perché questo blocco tornerà identico in una futura pagina 'aggiungi visita' su un paziente esistente") — espone `valida()`/`ottieniDati()` via `defineExpose`, pronto per essere riusato qui.
...
3. **Isolamento test in `PazienteRigaAzioni.spec.ts`**: 3 test falliscono solo se eseguiti insieme nel file — da investigare con `superpowers:systematic-debugging` se blocca la CI o se Andrea lo richiede. Segnalato per la terza volta consecutiva, mai affrontato.
