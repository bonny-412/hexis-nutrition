...
Andrea ha annunciato la prossima sessione: **modifica anagrafica paziente** (rotta `/pazienti/:id/modifica`, oggi un bottone disabilitato) e **inserimento nuova visita su paziente esistente** (rotta `/pazienti/:id/nuova-visita`, oggi un bottone disabilitato in questa view, voce di menu disabilitata nella lista pazienti fin dal 2 settembre) — due sotto-progetti distinti, probabilmente da affrontare uno alla volta. Dettagli su scope e comportamento non ancora raccolti, da chiedere a inizio sessione via `superpowers:brainstorming` (percorso architetturale quasi certo per "nuova visita", che riusa `DatiVisitaForm.vue` ma richiede un nuovo endpoint backend `POST /pazienti/{id}/visite`; "modifica anagrafica" potrebbe essere bounded o architetturale a seconda di quanti campi Andrea vuole editabili).
