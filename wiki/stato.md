...
Andrea ha annunciato la prossima sessione: lavorare sulla **pagina di dettaglio del paziente** (`views/pazienti/PazienteDettaglioView.vue`) — dettagli su cosa cambiare non ancora raccolti, da chiedere a inizio sessione (probabilmente via `superpowers:brainstorming`, dato che si parla di una pagina esistente ma il tipo di modifica — bounded o architetturale — dipende da cosa Andrea porta).
...
3. **Isolamento test in `PazienteRigaAzioni.spec.ts`**: 3 test falliscono solo se eseguiti insieme nel file (indipendentemente dal lavoro di questa sessione, vedi sopra) — da investigare con `superpowers:systematic-debugging` se blocca la CI o se Andrea lo richiede.
...
7. Colmare il gap di validazione server-side su `CreaPazienteRequest` per nome/cognome/telefono/lavoro (vedi sopra), se Andrea lo ritiene prioritario.
