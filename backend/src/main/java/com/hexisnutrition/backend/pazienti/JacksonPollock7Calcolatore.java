...
public class JacksonPollock7Calcolatore implements CalcolatorePlicometria {
    ...
    public RisultatoDensita calcola(PlicheInput pliche, ContestoPlicometria contesto) {
        double s7 = CalcoliPlicometria.somma(
                pliche.pettoraleMm(), 
                pliche.ascellareMm(), 
                pliche.tricipitaleMm(),
                pliche.sottoscapolareMm(), 
                pliche.addominaleMm(), 
                pliche.soprailiacaMm(), 
                pliche.cosciaMm()
        );

        double eta = contesto.etaAnni();

        // Nota l'uso di 5.5e-7 e 5.6e-7 per garantire la corretta notazione scientifica
        double d = contesto.sesso() == Sesso.M
                ? 1.112 - 0.00043499 * s7 + 5.5e-7 * s7 * s7 - 0.00028826 * eta
                : 1.097 - 0.00046971 * s7 + 5.6e-7 * s7 * s7 - 0.00012828 * eta;

        double percentualeGrasso = CalcoliPlicometria.percentualeGrassoSiri(d);

        return new RisultatoDensita(
                CalcoliPlicometria.arrotonda(s7, 2), 
